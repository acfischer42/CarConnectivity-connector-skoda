...
from .util_override import log_extra_keys
...
class Connector(BaseConnector):
            ...
            if 'vehicles' in data and data['vehicles'] is not None:
                for vehicle_dict in data['vehicles']:
                    if 'vin' in vehicle_dict and vehicle_dict['vin'] is not None:
                        ...
                        # expose raw API dictionary for debugging / compatibility checks
                        try:
                            safe = config_remove_credentials(vehicle_dict.copy())
                            vehicle.raw_api._set_value(value=json.dumps(safe, default=str, ensure_ascii=False))  # pylint: disable=protected-access
                        except Exception:
                            # best-effort: don't fail parsing due to raw dump
                            pass

                        # log and store unexpected keys
                        log_extra_keys(LOG_API, 'vehicles', vehicle_dict,  {'vin', 'licensePlate', 'name', 'title', 'devicePlatform', 'renders', 'priority', 'compositeRenders', 'state', 'systemModelId'})
                        try:
                            self._store_extra(vehicle, 'vehicles', vehicle_dict, {'vin', 'licensePlate', 'name', 'title', 'devicePlatform', 'renders', 'priority', 'compositeRenders', 'state', 'systemModelId'})
                        except Exception:
                            pass

                        # additional top-level fields
                        if 'renders' in vehicle_dict and vehicle_dict['renders'] is not None:
                            try:
                                vehicle.renders._set_value(json.dumps(vehicle_dict['renders'], default=str, ensure_ascii=False))  # pylint: disable=protected-access
                            except Exception:
                                vehicle.renders._set_value(None)  # pylint: disable=protected-access
                        else:
                            vehicle.renders._set_value(None)  # pylint: disable=protected-access
                        if 'compositeRenders' in vehicle_dict and vehicle_dict['compositeRenders'] is not None:
                            try:
                                vehicle.composite_renders._set_value(json.dumps(vehicle_dict['compositeRenders'], default=str, ensure_ascii=False))  # pylint: disable=protected-access
                            except Exception:
                                vehicle.composite_renders._set_value(None)  # pylint: disable=protected-access
                        else:
                            vehicle.composite_renders._set_value(None)  # pylint: disable=protected-access
                        # extract composite render urls
                        try:
                            urls = []
                            if 'compositeRenders' in vehicle_dict and vehicle_dict['compositeRenders'] is not None:
                                for cr in vehicle_dict['compositeRenders']:
                                    if 'layers' in cr and cr['layers'] is not None:
                                        for layer in cr['layers']:
                                            if 'url' in layer and layer['url'] is not None:
                                                urls.append(layer['url'])
                            if len(urls) > 0:
                                vehicle.composite_render_urls._set_value(json.dumps(urls, ensure_ascii=False))  # pylint: disable=protected-access
                            else:
                                vehicle.composite_render_urls._set_value(None)  # pylint: disable=protected-access
                        except Exception:
                            try:
                                vehicle.composite_render_urls._set_value(None)  # pylint: disable=protected-access
                            except Exception:
        ...
        for vin in set(garage.list_vehicle_vins()):
            ...
            if vehicle_to_update is not None and isinstance(vehicle_to_update, SkodaVehicle) and vehicle_to_update.is_managed_by_connector(self):
                ...
                if vehicle_to_update.capabilities is not None and vehicle_to_update.capabilities.enabled:
                    ...
                    if (vehicle_to_update.capabilities.has_capability('ACTIVE_VENTILATION', check_status_ok=True) or 
                        vehicle_to_update.capabilities.has_capability('AIR_CONDITIONING', check_status_ok=True) or
                        vehicle_to_update.capabilities.has_capability('VENTILATION', check_status_ok=True) or
                        vehicle_to_update.capabilities.has_capability('CLIMATE_CONTROL', check_status_ok=True)):
        ...
        if data is not None:
            ...
            if 'status' in data and data['status'] is not None:
                ...
                if 'battery' in data['status'] and data['status']['battery'] is not None:
                    for drive in vehicle.drives.drives.values():
                        ...
                        if isinstance(drive, ElectricDrive):
                            ...
                            try:
                                self._store_extra(vehicle, 'status.battery', data['status']['battery'], {'remainingCruisingRangeInMeters', 'stateOfChargeInPercent'})
                            except Exception:
                                pass
                            break
                log_extra_keys(LOG_API, 'status', data['status'],  {'chargingRateInKilometersPerHour',
                                                                    ...
                                                                    'battery'})
                try:
                    self._store_extra(vehicle, 'status', data['status'], {'chargingRateInKilometersPerHour', 'chargePowerInKw', 'remainingTimeToFullyChargedInMinutes', 'state', 'chargeType', 'battery'})
                except Exception:
                    pass
            if 'settings' in data and data['settings'] is not None:
                ...
                try:
                    self._store_extra(vehicle, 'charging.settings', data['settings'], {'targetStateOfChargeInPercent', 'maxChargeCurrentAc', 'autoUnlockPlugWhenCharged', 'preferredChargeMode', 'availableChargeModes', 'chargingCareMode', 'batterySupport'})
                except Exception:
                    pass
    ...
    def fetch_maintenance(self, vehicle: SkodaVehicle, no_cache: bool = False) -> SkodaVehicle:
        ...
        if data is not None:
            ...
            # Add maintenance data to raw_api for debugging
            try:
                if vehicle.raw_api.value is not None:
                    raw_data = json.loads(vehicle.raw_api.value)
                else:
                    raw_data = {}
                raw_data['maintenance'] = data
                vehicle.raw_api._set_value(value=json.dumps(raw_data, default=str, ensure_ascii=False))  # pylint: disable=protected-access
            except Exception:
                # best-effort: don't fail parsing due to raw dump
                pass
    ...
    def fetch_air_conditioning(self, vehicle: SkodaVehicle, no_cache: bool = False) -> SkodaVehicle:
        """
        Fetches the air conditioning/ventilation data for a given Skoda vehicle and updates the vehicle object with the retrieved data.
        
        This method handles air conditioning, ventilation, and climate control systems, triggered by any of:
        ACTIVE_VENTILATION, AIR_CONDITIONING, VENTILATION, or CLIMATE_CONTROL capabilities.
        
        Different Skoda vehicle models may use different capability names:
        - ACTIVE_VENTILATION: Commonly used for newer models with advanced climate control
        - AIR_CONDITIONING: Traditional air conditioning systems
        - VENTILATION: Basic ventilation systems
        - CLIMATE_CONTROL: Comprehensive climate management systems
        ...
        active_capabilities = []
        for cap in ['ACTIVE_VENTILATION', 'AIR_CONDITIONING', 'VENTILATION', 'CLIMATE_CONTROL']:
            if vehicle.capabilities.has_capability(cap, check_status_ok=True):
                active_capabilities.append(cap)
        if active_capabilities:
            LOG_API.debug(f'Fetching air conditioning data for {vin} based on capabilities: {", ".join(active_capabilities)}')
        if vehicle.position is None:
            raise ValueError('Vehicle has no charging object')
        ...
        if data is not None:
            ...
            if 'outsideTemperature' in data and data['outsideTemperature'] is not None:
                ...
                if 'temperatureUnit' in data['outsideTemperature'] and data['outsideTemperature']['temperatureUnit'] is not None:
                    ...
                    try:
                        self._store_extra(vehicle, 'air-conditioning.outsideTemperature', data['outsideTemperature'], {'carCapturedTimestamp', 'temperatureUnit', 'temperatureValue'})
                    except Exception:
                        pass
            else:
                ...
                if isinstance(vehicle.climatization, SkodaClimatization):
                    ...
                    for timer in data['timers']:
                        if 'id' in timer:
                            ...
                            timer_obj.timer_enabled = BooleanAttribute(name='timer_enabled', parent=timer_obj)
                            timer_obj.time = StringAttribute(name='time', parent=timer_obj)
                            timer_obj.type = StringAttribute(name='type', parent=timer_obj)
                            timer_obj.selected_days = GenericAttribute(name='selected_days', parent=timer_obj)
            ...
            try:
                self._store_extra(vehicle, 'air-condition', data, {'carCapturedTimestamp', 'state', 'estimatedDateTimeToReachTargetTemperature', 'targetTemperature', 'outsideTemperature', 'chargerConnectionState', 'chargerLockState', 'airConditioningAtUnlock', 'steeringWheelPosition', 'windowHeatingEnabled', 'seatHeatingActivated', 'windowHeatingState', 'errors', 'runningRequests', 'timers'})
            except Exception:
                pass
        return vehicle
    ...
    def fetch_vehicle_details(self, vehicle: SkodaVehicle, no_cache: bool = False) -> SkodaVehicle:
        ...
        if vehicle_data:
            # store raw vehicle details for inspection
            try:
                safe = config_remove_credentials(vehicle_data.copy())
                vehicle.raw_api._set_value(value=json.dumps(safe, default=str, ensure_ascii=False))  # pylint: disable=protected-access
            except Exception:
                pass
            ...
            if 'specification' in vehicle_data and vehicle_data['specification'] is not None:
                ...
                log_extra_keys(LOG_API, 'specification', vehicle_data['specification'],  {'model', 'modelYear', 'title', 'manufacturingDate', 'body', 'systemCode', 'trimLevel', 'engine', 'exteriorDimensions', 'systemModelId', 'gearbox', 'exteriorColour'})
                try:
                    self._store_extra(vehicle, 'specification', vehicle_data['specification'], {'model', 'modelYear', 'title', 'manufacturingDate', 'body', 'systemCode', 'trimLevel', 'engine', 'exteriorDimensions', 'systemModelId', 'gearbox', 'exteriorColour'})
                except Exception:
                    pass
                # manufacturing date: try parsing into datetime attribute
                try:
                    if 'manufacturingDate' in vehicle_data['specification'] and vehicle_data['specification']['manufacturingDate'] is not None:
                        mfg_dt = robust_time_parse(vehicle_data['specification']['manufacturingDate'])
                        vehicle.specification.manufacturing_date._set_value(value=mfg_dt)  # pylint: disable=protected-access
                    else:
                        vehicle.specification.manufacturing_date._set_value(None)  # pylint: disable=protected-access
                except Exception:
                    # best-effort; leave as None on parse errors
                    try:
                        vehicle.specification.manufacturing_date._set_value(None)  # pylint: disable=protected-access
                    except Exception:
                        pass
                # model year: try parsing into string attribute
                try:
                    if 'modelYear' in vehicle_data['specification'] and vehicle_data['specification']['modelYear'] is not None:
                        vehicle.specification.model_year._set_value(str(vehicle_data['specification']['modelYear']))  # pylint: disable=protected-access
                    else:
                        vehicle.specification.model_year._set_value(None)  # pylint: disable=protected-access
                except Exception:
                    # best-effort; leave as None on parse errors
                    try:
                        vehicle.specification.model_year._set_value(None)  # pylint: disable=protected-access
                        ...
                        pass
                # title: try parsing into string attribute
                try:
                    if 'title' in vehicle_data['specification'] and vehicle_data['specification']['title'] is not None:
                        vehicle.specification.title._set_value(str(vehicle_data['specification']['title']))  # pylint: disable=protected-access
                    else:
                        vehicle.specification.title._set_value(None)  # pylint: disable=protected-access
                except Exception:
                    try:
                        vehicle.specification.title._set_value(None)  # pylint: disable=protected-access
                    except Exception:
                        pass
                # body: try parsing into string attribute
                try:
                    if 'body' in vehicle_data['specification'] and vehicle_data['specification']['body'] is not None:
                        vehicle.specification.body._set_value(str(vehicle_data['specification']['body']))  # pylint: disable=protected-access
                    else:
                        vehicle.specification.body._set_value(None)  # pylint: disable=protected-access
                except Exception:
                    try:
                        vehicle.specification.body._set_value(None)  # pylint: disable=protected-access
                    except Exception:
                        pass
                # exterior colour: try parsing into string attribute
                try:
                    if 'exteriorColour' in vehicle_data['specification'] and vehicle_data['specification']['exteriorColour'] is not None:
                        vehicle.specification.exterior_colour._set_value(str(vehicle_data['specification']['exteriorColour']))  # pylint: disable=protected-access
                    else:
                        vehicle.specification.exterior_colour._set_value(None)  # pylint: disable=protected-access
                except Exception:
                    try:
                        vehicle.specification.exterior_colour._set_value(None)  # pylint: disable=protected-access
                    except Exception:
                        pass
                # trim level: try parsing into string attribute
                try:
                    if 'trimLevel' in vehicle_data['specification'] and vehicle_data['specification']['trimLevel'] is not None:
                        vehicle.specification.trim_level._set_value(str(vehicle_data['specification']['trimLevel']))  # pylint: disable=protected-access
                    else:
                        vehicle.specification.trim_level._set_value(None)  # pylint: disable=protected-access
                except Exception:
                    try:
                        vehicle.specification.trim_level._set_value(None)  # pylint: disable=protected-access
                    except Exception:
                        pass
                # system code: try parsing into string attribute
                try:
                    if 'systemCode' in vehicle_data['specification'] and vehicle_data['specification']['systemCode'] is not None:
                        vehicle.specification.system_code._set_value(str(vehicle_data['specification']['systemCode']))  # pylint: disable=protected-access
                    else:
                        vehicle.specification.system_code._set_value(None)  # pylint: disable=protected-access
                except Exception:
                    try:
                        vehicle.specification.system_code._set_value(None)  # pylint: disable=protected-access
                    except Exception:
                        pass
                # system model id: try parsing into string attribute
                try:
                    if 'systemModelId' in vehicle_data['specification'] and vehicle_data['specification']['systemModelId'] is not None:
                        vehicle.specification.system_model_id._set_value(str(vehicle_data['specification']['systemModelId']))  # pylint: disable=protected-access
                    else:
                        vehicle.specification.system_model_id._set_value(None)  # pylint: disable=protected-access
                except Exception:
                    try:
                        vehicle.specification.system_model_id._set_value(None)  # pylint: disable=protected-access
                    except Exception:
                        pass
                # exterior dimensions: try to parse into mm values if present
                try:
                    if 'exteriorDimensions' in vehicle_data['specification'] and vehicle_data['specification']['exteriorDimensions'] is not None:
                        ext = vehicle_data['specification']['exteriorDimensions']
                        # store original
                        # populate nested exteriorDimensions fields matching JSON structure
                        if isinstance(ext, dict):
                            try:
                                if 'lengthInMm' in ext and ext['lengthInMm'] is not None:
                                    vehicle.specification.exteriorDimensions.lengthInMm._set_value(str(ext['lengthInMm']))  # pylint: disable=protected-access
                                else:
                                    vehicle.specification.exteriorDimensions.lengthInMm._set_value(None)  # pylint: disable=protected-access
                            except Exception:
                                vehicle.specification.exteriorDimensions.lengthInMm._set_value(None)  # pylint: disable=protected-access
                            
                            try:
                                if 'widthInMm' in ext and ext['widthInMm'] is not None:
                                    vehicle.specification.exteriorDimensions.widthInMm._set_value(str(ext['widthInMm']))  # pylint: disable=protected-access
                                else:
                                    vehicle.specification.exteriorDimensions.widthInMm._set_value(None)  # pylint: disable=protected-access
                            except Exception:
                                vehicle.specification.exteriorDimensions.widthInMm._set_value(None)  # pylint: disable=protected-access
                            
                            try:
                                if 'heightInMm' in ext and ext['heightInMm'] is not None:
                                    vehicle.specification.exteriorDimensions.heightInMm._set_value(str(ext['heightInMm']))  # pylint: disable=protected-access
                                else:
                                    vehicle.specification.exteriorDimensions.heightInMm._set_value(None)  # pylint: disable=protected-access
                            except Exception:
                                vehicle.specification.exteriorDimensions.heightInMm._set_value(None)  # pylint: disable=protected-access
                    else:
                        # clear nested exteriorDimensions fields
                        vehicle.specification.exteriorDimensions.lengthInMm._set_value(None)  # pylint: disable=protected-access
                        vehicle.specification.exteriorDimensions.widthInMm._set_value(None)  # pylint: disable=protected-access
                        vehicle.specification.exteriorDimensions.heightInMm._set_value(None)  # pylint: disable=protected-access
                except Exception:
                    # best-effort - clear all exteriorDimensions fields
                    try:
                        vehicle.specification.exteriorDimensions.lengthInMm._set_value(None)  # pylint: disable=protected-access
                        vehicle.specification.exteriorDimensions.widthInMm._set_value(None)  # pylint: disable=protected-access
                        vehicle.specification.exteriorDimensions.heightInMm._set_value(None)  # pylint: disable=protected-access
                        ...
                        pass
                # engine: parse power, displacement, fuel type if present (handle multiple key variants)
                try:
                    if 'engine' in vehicle_data['specification'] and vehicle_data['specification']['engine'] is not None:
                        eng = vehicle_data['specification']['engine']
                        # populate nested engine fields matching JSON structure
                        if isinstance(eng, dict):
                            try:
                                if 'type' in eng and eng['type'] is not None:
                                    vehicle.specification.engine.type._set_value(str(eng['type']))  # pylint: disable=protected-access
                                else:
                                    vehicle.specification.engine.type._set_value(None)  # pylint: disable=protected-access
                            except Exception:
                                vehicle.specification.engine.type._set_value(None)  # pylint: disable=protected-access
                            
                            try:
                                if 'powerInKW' in eng and eng['powerInKW'] is not None:
                                    vehicle.specification.engine.powerInKW._set_value(str(eng['powerInKW']))  # pylint: disable=protected-access
                                else:
                                    vehicle.specification.engine.powerInKW._set_value(None)  # pylint: disable=protected-access
                            except Exception:
                                vehicle.specification.engine.powerInKW._set_value(None)  # pylint: disable=protected-access
                            
                            try:
                                if 'capacityInLiters' in eng and eng['capacityInLiters'] is not None:
                                    vehicle.specification.engine.capacityInLiters._set_value(str(eng['capacityInLiters']))  # pylint: disable=protected-access
                                else:
                                    vehicle.specification.engine.capacityInLiters._set_value(None)  # pylint: disable=protected-access
                            except Exception:
                                vehicle.specification.engine.capacityInLiters._set_value(None)  # pylint: disable=protected-access


                    else:
                        # clear nested engine fields
                        vehicle.specification.engine.type._set_value(None)  # pylint: disable=protected-access
                        vehicle.specification.engine.powerInKW._set_value(None)  # pylint: disable=protected-access
                        vehicle.specification.engine.capacityInLiters._set_value(None)  # pylint: disable=protected-access
                except Exception:
                    # best-effort - clear all engine fields
                    try:
                        vehicle.specification.engine.type._set_value(None)  # pylint: disable=protected-access
                        vehicle.specification.engine.powerInKW._set_value(None)  # pylint: disable=protected-access
                        vehicle.specification.engine.capacityInLiters._set_value(None)  # pylint: disable=protected-access
                        ...
                        pass
                # gearbox: parse gearbox information if present
                try:
                    if 'gearbox' in vehicle_data['specification'] and vehicle_data['specification']['gearbox'] is not None:
                        gbox = vehicle_data['specification']['gearbox']
                        # populate nested gearbox fields matching JSON structure
                        if isinstance(gbox, dict):
                            try:
                                if 'type' in gbox and gbox['type'] is not None:
                                    vehicle.specification.gearbox.type._set_value(str(gbox['type']))  # pylint: disable=protected-access
                                else:
                                    vehicle.specification.gearbox.type._set_value(None)  # pylint: disable=protected-access
                            except Exception:
                                vehicle.specification.gearbox.type._set_value(None)  # pylint: disable=protected-access
                    else:
                        # clear nested gearbox fields
                        vehicle.specification.gearbox.type._set_value(None)  # pylint: disable=protected-access
                except Exception:
                    # best-effort - clear all gearbox fields
                    try:
                        vehicle.specification.gearbox.type._set_value(None)  # pylint: disable=protected-access
                    except Exception:
                        pass
                ...
                vehicle.model._set_value(None)  # pylint: disable=protected-access
            log_extra_keys(LOG_API, 'api/v2/garage/vehicles/VIN', vehicle_data, {'softwareVersion', 'devicePlatform', 'renders', 'name', 'specification', 'vin', 'servicePartner', 'compositeRenders', 'state', 'capabilities', 'workshopModeEnabled'})
            try:
                self._store_extra(vehicle, 'api/v2/garage/vehicles/VIN', vehicle_data, {'softwareVersion', 'devicePlatform', 'renders', 'name', 'specification', 'vin', 'servicePartner', 'compositeRenders', 'state', 'capabilities', 'workshopModeEnabled'})
            except Exception:
                pass
            # service partner
            if 'servicePartner' in vehicle_data and vehicle_data['servicePartner'] is not None:
                try:
                    if 'servicePartnerId' in vehicle_data['servicePartner'] and vehicle_data['servicePartner']['servicePartnerId'] is not None:
                        vehicle.service_partner.service_partner_id._set_value(str(vehicle_data['servicePartner']['servicePartnerId']))  # pylint: disable=protected-access
                    else:
                        vehicle.service_partner.service_partner_id._set_value(None)  # pylint: disable=protected-access
                except Exception:
                    vehicle.service_partner.service_partner_id._set_value(None)  # pylint: disable=protected-access
            else:
                vehicle.service_partner.service_partner_id._set_value(None)  # pylint: disable=protected-access
        ...
        if vehicle_status_data:
            ...
            if 'overall' in vehicle_status_data and vehicle_status_data['overall'] is not None:
                ...
                log_extra_keys(LOG_API, 'status overall', vehicle_status_data['overall'], {'doorsLocked',
                                                                                           ...
                                                                                           'windows',
                                                                                           'lights'})
            log_extra_keys(LOG_API, f'/api/v2/vehicle-status/{vin}', vehicle_status_data, {'overall', 'carCapturedTimestamp', 'renders', 'detail'})
        return vehicle
    ...
    def _store_extra(self, vehicle: SkodaVehicle, where: str, data: Dict[str, Any], allowed_keys: set) -> None:
        """
        Merge unexpected keys from a response dictionary into vehicle.extras as a JSON mapping.

        - Filters out allowed_keys.
        - Removes credentials via config_remove_credentials.
        - Merges into existing JSON stored in vehicle.extras (shallow merge by key).
        Best-effort only; exceptions are caught where called.
        """
        if vehicle is None or data is None:
            return
        try:
            extras_map: Dict[str, Any] = {}
            # start from existing extras if present and valid JSON
            if hasattr(vehicle, 'extras') and vehicle.extras is not None and vehicle.extras.enabled and vehicle.extras.value is not None:
                try:
                    extras_map = json.loads(vehicle.extras.value)
                except Exception:
                    extras_map = {}

            # compute unexpected keys and add them
            for key, val in data.items():
                if key in allowed_keys:
                    continue
                try:
                    # sanitize value
                    safe_val = config_remove_credentials({key: val})[key]
                except Exception:
                    safe_val = val
                extras_map.setdefault(where, {})
                extras_map[where][key] = safe_val

            # write back
            if hasattr(vehicle, 'extras') and vehicle.extras is not None:
                vehicle.extras._set_value(value=json.dumps(extras_map, default=str, ensure_ascii=False))  # pylint: disable=protected-access
        except Exception:
